...
class RemoteComander:
    ...
    def conectar(self):
        ...
        try:
            ...
            print(f'❌ Erro ao configura sessão com IP: {self.ip} | log: {e}')
            return False
    ...
    def executar_powershell(self, comando):
        ...
        try:
            ...
            print(f'📡 Enviando sinal de comando para {self.ip}, aguarde...')
            resultado = self.session.run_ps(comando)
            
            # Decodifica a saida (WinRM retorna bytes)
            ...
            if resultado.status_code == 0:
                ...
                return {'status':'erro', 'saida': f'Erro (Saida de Código {resultado.status_code}): {std_err}'}
        except (ConnectTimeout, ConnectionError):
            return {'status':'erro', 'saida':f'Timeout: O Servidor {self.ip}, não respondeu (WinRM 5985).'}
        except Exception as e:
